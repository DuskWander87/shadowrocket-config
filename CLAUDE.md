...
- 归属主体无法确认、但 NS 在国内托管商（阿里云 / 腾讯 DNSPod / 华为云等）且无抢注特征的域名，可作特例收录至 `ChinaDirect.list` 末尾「归属未确认的特例」分区，须记录收录理由与 NS 托管商，待归属明确后迁移至上方对应分组或移除。核查过程中的 DNS 快照（TXT / SOA / 解析 IP 等）不入档——易过期，且复核时必然重查，详见「规则注释规范」。
...
| 归属未确认标注 | `# Washpayer 自助洗护扫码支付平台 (运营主体未确认，阿里云托管)` | 提示后续需复核 |
| 末尾特例区说明 | 分区标题 + 收录理由 + NS 托管商 | 见「新增直连域名前必须验证归属」的黄线要求 |
